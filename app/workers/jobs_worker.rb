...
class JobsWorker
  ...
  def perform(job_id, server_id)
    ...
    if job.is_running == false
      ...
      Open3.popen3("ssh -2 -4 -o ConnectTimeout=3 #{server.login}@#{server.ip} #{server.gptlauncher_path}/launcher.py
                    --job #{job.jobtype.name} --config #{server.name}
                    --territory #{job.territory.name}
                    --version #{job.version}
                    &> /tmp/gpt_launcher.err"
                  ) {|input,out,err,wait_thr|
